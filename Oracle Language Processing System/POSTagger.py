...
def Main():
	...
	i = 0
	while i < 2:
		sent = input("Please enter a sentence to tag: ")
		sent = sent.split()
		print(test_tagger(taggers[0], sent))
		i += 1
	
	print("Serializing tagger for later use, please wait...")
	serialize_object(taggers[5], 'tag', 'default_tagger.pkl')
	serialize_object(taggers[4], 'tag', 'unigram_tagger.pkl')
	serialize_object(taggers[3], 'tag', 'bigram_tagger.pkl')
	serialize_object(taggers[2], 'tag', 'trigram_tagger.pkl')
	serialize_object(taggers[1], 'tag', 'quadrigram_tagger.pkl')
	serialize_object(taggers[0], 'tag', 'quintigram_tagger.pkl')
