...
class Spider(HTMLParser):
    def __init__(self, URL, maxpages=200):
        super(Spider, self).__init__()
        self.wait_queue = [] 
        self.baseUrl = None
        #self.search_phrase = None
        self.search_span = 0
        input_test = self.change_base_URL(URL, maxpages)
        if input_test != 0:
        	print("Invalid Input Parameters.")
        	return
        self.spider_log = open(os.getcwd() + "/Logs/Spider Logs/spiderlog_" + str(datetime.datetime.today()) + ".txt", "w")
        self.links_visited = []
    ...
    def check_visit_status(self, newURL):
        if newURL not in self.wait_queue and newURL not in self.links_visited and bool(urlparse(newURL).netloc):
            return True
        else:
            return False
    ...
    def change_base_URL(self, url=None, maxpages=200, grep_options=""):
        # Rprint("First Queue on wait queue: ", self.wait_queue[0])etains Old Settings
    ...
    def readPage(self):
        #random_index = randrange(0,self.LinksCount())
        #url = self.wait_queue[random_index]
        url = self.wait_queue[0]
        print(str(self.num_links_visited) + " Visiting:" + url + ".\n")
        ...
        except (NameError) as e:                                    # urllib.error.HTTPError, 
    ...
    def crawl(self):  
        ...
        print("Crawling...\n")
        if self.num_links_visited < self.search_span and self.LinksCount != 0:
            try:
                #url = self.wait_queue[0]
                data = self.readPage()
        ...
        return data
            #print("First Queue on wait queue: ", self.wait_queue[0])
    ...
    def VisitingLink(self, index):
    	self.links_visited = self.links_visited + [self.wait_queue[index]]
    	del self.wait_queue[index]
    	if self.wait_queue == []:
