...
class Spider(HTMLParser):
            ...
            for (key, value) in attrs:
                if key == 'href':
                    if "(" in value and ")" in value:
                        ...
                        self.spider_log = self.spider_log + "Found New URL: " + newURL + ".\n"
                    else:
                        ...
                        self.spider_log = self.spider_log + "Found New URL: Resolving " + self.wait_queue[0] + " " + urlparse(value).path + "To : " + newURL + ".\n"
    ...
    def readPage(self):
        ...
        if  bool(urlparse(url).netloc) and urlparse(url).netloc != self.baseUrl:     # change base url if visitng new host site
            ...
            self.spider_log = self.spider_log + "Updating Spider.baseUrl from " + self.baseUrl + " to " + urlparse(url).netloc + ".\n"
            self.baseUrl = urlparse(url).netloc
        try:
            ...
            self.spider_log = self.spider_log + "Exception: " + str(e) + ".\n"
            print("Nothing there...")
        except:
            self.spider_log = self.spider_log + "Exception: Other Exception: " + str(sys.exc_info()[0]) + ".\n"
    ...
    def crawl(self):  
        foundWord = False
        self.spider_log = self.spider_log + "Crawling...\n"
        print("Crawling...\n")
        while self.num_links_visited < self.search_span and self.LinksCount != 0 and not foundWord:
            try:
                ...
                if data.find(self.search_phrase)>-1:
                    ...
                    self.spider_log = self.spider_log + "**Success!** : Number of pages visited: " + self.num_links_visited + ".\n"      
            except (NameError) as e:
                self.spider_log = self.spider_log + "Exception: " + str(e) + ".\n"
            except:
                print("**Failed on URL: **" + url + ".\n")
                self.spider_log = self.spider_log + "**Failed on URL: **" + url + " Exception : " + str(sys.exc_info()[0]) + ".\n"
        ...
        if foundWord:
            ...
            self.spider_log = self.spider_log + "The word " + self.search_phrase + " was found at " + url + ".\n"

        else:
            print("**Failed!**.\n")
            print("Word never found. Number of Pages Visited: " + self.num_links_visited + ".\n")
            self.spider_log = self.spider_log + "**Failed!**.\n"
            self.spider_log = self.spider_log + "Word never found. Number of Pages Visited: " + self.num_links_visited + ".\n"
    ...
    def VisitingLink(self):
    	...
    	if self.wait_queue == []:
            ...
            self.spider_log = self.spider_log + "No Where Left To Crawl!.\n"
            sys.exit()
    ...
    def Write_Stats(self):
        self.spider_log = self.spider_log + "\n******************Spider Stats******************.\n"
        self.spider_log = self.spider_log + "------------------------------------------------.\n"
        self.spider_log = self.spider_log + "Spider Execution Time: " + str(self.execution_time) + " seconds.\n"
        self.spider_log = self.spider_log + "Spiders Current Word: " + self.search_phrase + ".\n"
        self.spider_log = self.spider_log + "Spiders Current Location: " + self.links_visited[-1] + ".\n"
        self.spider_log = self.spider_log + "Spider Depth: " + str(self.LinksVisitedCount()) + ".\n"
        self.spider_log = self.spider_log + "Spiders Links Left To Visit Count: " + str(self.LinksCount()) + ".\n"
        self.spider_log = self.spider_log + "------------------------------------------------\n"
        self.spider_log = self.spider_log + "Links Left To Visit:\n" 
        for i in self.wait_queue:
            self.spider_log = self.spider_log + i + ".\n"
        self.spider_log = self.spider_log + "------------------------------------------------\n"
        file_object = open(os.getcwd() + "/Spider Logs/spiderlog_" + str(datetime.datetime.today()) + ".txt", "w")
        file_object.write(self.spider_log)
        file_object.close()
